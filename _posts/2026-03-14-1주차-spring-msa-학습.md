...
- N:M (다대다) 관계: 복잡성이 높아 실무에서는 기피합니다. 대신 중간에 Mapping Table을 두어 1:N 관계로 풀어내는 것이 조회 속도와 관리 면에서 훨씬 빠릅니다.
...
질문하신 "Join FETCH table..? 아니면 해당 객체 명..??"에 대한 답은 **무조건 '엔티티(객체) 명과 필드 명'**을 사용해야 합니다. JPQL은 DB 테이블이 아니라 Java 객체를 대상으로 쿼리를 짜기 때문입니다.

예시:

```sql
- 테이블 명이 아닌, Product 엔티티(p)와 그 안의 category 필드를 사용합니다.SELECT p FROM Product p JOIN FETCH p.category
```


---

## N+1 해결 전략: Batch Size (default_batch_fetch_size)

설명:

Fetch Join을 쓸 수 없는 상황(예: 페이징 처리)에서 N+1을 완화하는 2순위 방법입니다. 질문하신 "batchSize...?? LAZY...?? EAGER...??"에 대한 답은, LAZY 로딩 환경에서 동작하는 최적화 옵션입니다. N번 쿼리가 나갈 것을 지정한 사이즈(보통 100~1000)만큼 모아서 SQL의 IN 쿼리 한 방으로 묶어서 쳐줍니다.
...
오늘 정리하신 내용을 바탕으로, 이전에 설계한 Product와 Category 엔티티를 활용하여 N+1 문제를 방지하는 Fetch Join이 적용된 ProductRepository 인터페이스 코드를 바로 작성해 볼까요?
