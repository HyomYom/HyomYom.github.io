...
`Git log`는 옵션이 매우 다양하고 조합에 따라 출력 결과도 크게 달라진다.
...
🔗 Git 공식 문서 : [https://git-scm.com/docs/git-log](https://git-scm.com/docs/git-log)
...
- `-oneline`
    → 커밋을 한 줄 요약으로 표시 (해시 + 메시지)

- `-graph`
    → 브랜치/머지 관계를 ASCII 그래프로 시각화

- `-decorate`
    → 브랜치명, 태그 등 참조 정보 표시

- `-all`
    → 현재 브랜치뿐 아니라 모든 브랜치 히스토리 표시
...
→ `origin/main`에는 없고 **현재 브랜치에만 있는 커밋**
...
| 목적 | 명령어 |
| --- | --- |
| 전체 흐름 보기 | `git log --oneline --graph --decorate --all` |
| 최근 내 작업 | `git log --author="me" -5` |
| 코드 변경 확인 | `git log -p` |
| 파일 히스토리 | `git log --follow 파일명` |
| 메인 브랜치 흐름 | `git log --first-parent` |
| 특정 코드 추적 | `git log -L` |
